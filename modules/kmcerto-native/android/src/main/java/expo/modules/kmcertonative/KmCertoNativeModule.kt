...
class KmCertoAccessibilityService : AccessibilityService() {
  ...
  override fun onAccessibilityEvent(event: AccessibilityEvent?) {
    ...
    // CORREÇÃO: não filtra por packageName aqui — o popup de oferta do Uber/iFood/99
    // aparece como janela de sistema (packageName = "android" ou vazio), não do app.
    // O parser já descarta textos sem R$, km e "aceitar" naturalmente.
    try {
      ...
      if (allWindows != null && allWindows.isNotEmpty()) {
        for (window in allWindows) {
          ...
          if (windowText.isNotBlank()) {
            ...
            // Guarda o pacote detectado se for suportado
        ...
        // Considera encontrado se coletou qualquer texto (o parser filtra o resto)
        if (allParts.isNotEmpty()) foundSupportedWindow = true
      }
    } catch (e: Throwable) {
