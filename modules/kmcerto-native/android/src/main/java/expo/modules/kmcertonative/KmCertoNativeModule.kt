...
// =====================================================================
// RUNTIME — Configurações e estado global
// =====================================================================
object KmCertoRuntime {
  ...
  // ── CORREÇÃO: Adicionados mais apps + label correto da 99 ──
...
// =====================================================================
// DATA CLASS — Dados da decisão sobre a oferta
// =====================================================================
...
object KmCertoOfferParser {
  ...
  // ── Palavras que indicam tela NÃO é oferta ────────────────────────
  private val nonOfferKeywords = listOf(
    ...
    "tendências de ganhos", "ganhos do dia", "resumo",
    "avaliação", "rating", "configurações", "settings"
  )

  // ── Palavras que indicam tela de OFERTA de corrida (ampliadas) ────
  // CORREÇÃO: Adicionadas palavras em inglês (Uber pode mostrar em EN)
  // e mais variantes em português
  ...
  fun parse(context: Context?, rawText: String, minimumPerKm: Double, sourcePackage: String): OfferDecisionData? {
    ...
    // CORREÇÃO: Não bloquear se o texto tem R$ + km de um app suportado
    ...
    // ── 1. Extrair valor R$ ─────────────────────────────────────────
    // CORREÇÃO: Pegar todos os valores R$ e escolher o mais provável (menor valor razoável)
    ...
    // Filtrar valores razoáveis para corrida (R$1,50 a R$500)
    val reasonableFares = allFareMatches.filter { it in 1.50..500.0 }
    // Pegar o primeiro valor razoável (geralmente é o valor da corrida)
    ...
    // Fallback 2: Se ainda não achou, tentar o menor valor encontrado
    if (fare == null) {
    ...
    val distances = kmRegex.findAll(normalizedText)
      .mapNotNull { it.groupValues.getOrNull(1)?.let(::parsePtBrNumber) }
      .filter { it > 0 && it.isFinite() && it < 200 } // Filtrar km absurdos
      .toList()
    ...
    val totalMinutes = minuteRegex.findAll(normalizedText)
      ...
      .filter { it > 0 && it < 300 } // Filtrar minutos absurdos
      .toList()
    ...
    // CORREÇÃO: Status em UPPERCASE para compatibilidade com TypeScript
...
// =====================================================================
// ACCESSIBILITY SERVICE — Leitura de tela CORRIGIDA
// =====================================================================
class KmCertoAccessibilityService : AccessibilityService() {
  ...
  override fun onServiceConnected() {
    ...
    // ── Configurar o serviço para ler TODAS as janelas ──────────────
  ...
  // =================================================================
  // CORREÇÃO PRINCIPAL: onAccessibilityEvent reescrito
  // =================================================================
  override fun onAccessibilityEvent(event: AccessibilityEvent?) {
    ...
    // ── CORREÇÃO: Ignorar eventos do próprio app (evita loop) ───────
    val ownPackage = packageName ?: ""
    if (eventPackage == ownPackage || eventPackage == "com.kmcerto.app") return

    // ── CORREÇÃO: Ignorar pacotes do sistema que não são relevantes ─
    ...
    // =================================================================
    // ESTRATÉGIA PRINCIPAL: Coletar texto de janelas de apps suportados
    // =================================================================
    val allParts = mutableListOf<String>()
    var detectedPackage = ""
    var foundSupportedContent = false

    // ESTRATÉGIA 1: Iterar sobre TODAS as janelas visíveis
    try {
      ...
      if (allWindows != null && allWindows.isNotEmpty()) {
        for (window in allWindows) {
          ...
          // ── CORREÇÃO: Só coletar texto de janelas de apps suportados ──
          // OU de janelas tipo OVERLAY/POPUP (que podem ser da Uber/99)
          ...
          if (isSupportedApp || isOverlayWindow || isAndroidOverlay) {
            val windowText = collectAllText(windowRoot)
            if (windowText.isNotBlank()) {
              allParts.add(windowText)
              Log.d(TAG, "WIN[$windowPkg type=${window.type}] text(200): ${windowText.take(200)}")
    ...
    // ESTRATÉGIA 2 (fallback): rootInActiveWindow + event source
    if (!foundSupportedContent) {
      // Se o evento veio de um app suportado, processar
      ...
      if (isEventFromSupported) {
        ...
        // Evento de app não suportado — verificar se é um overlay com conteúdo de corrida
        // (Uber pode mostrar overlay com packageName "android")
        if (isAndroidSystemPackage(eventPackage)) {
          ...
          if (source != null) {
            ...
            if (sourceText.isNotBlank() && looksLikeRideOffer(sourceText)) {
              ...
              // Tentar detectar qual app é baseado no conteúdo
              detectedPackage = guessSourceApp(sourceText)
              foundSupportedContent = true
            }
            try { source.recycle() } catch (_: Throwable) { }
          }
    ...
    // ── CORREÇÃO: Filtro inteligente baseado em R$ + km ─────────────
    // Em vez de exigir "aceitar/selecionar", verificar se tem R$ E km
    // O parser já faz a validação completa
    ...
    if (detectedPackage.isBlank() || !KmCertoRuntime.supportsPackage(detectedPackage)) {
      if (KmCertoRuntime.supportsPackage(eventPackage)) {
        ...
        // Último fallback: tentar adivinhar pelo conteúdo
    ...
    // ── CORREÇÃO: Debounce aumentado para 6 segundos ────────────────
  ...
  // ── Verificar se é pacote do sistema Android ──────────────────────
  ...
  private fun looksLikeRideOffer(text: String): Boolean {
    ...
    val hasR = lower.contains("r$")
    val hasKm = lower.contains("km")
    return hasR && hasKm
  }

  // ── Adivinhar app de origem baseado no conteúdo do texto ──────────
  private fun guessSourceApp(text: String): String {
    ...
    return when {
      ...
      lower.contains("99") || lower.contains("99pop") || lower.contains("99taxi") -> "com.app99.driver"
  ...
  // ── Coletar todo o texto de uma árvore de nós ─────────────────────
...
// =====================================================================
// OVERLAY SERVICE — CORREÇÃO: Card compacto no topo (não bloqueia tela)
// =====================================================================
class KmCertoOverlayService : Service() {
  ...
  // ── CORREÇÃO: Overlay compacto no TOPO da tela (não bloqueia) ─────
  private fun showOverlayInternal(data: OfferDecisionData) {
    ...
    val bgColor = if (isAccept) "#1A2E1A" else "#2E1A1A"
    ...
    // ── Linha 2: Métricas R$/km | R$/hr | R$/min ─────────────────────
    ...
    // ── CORREÇÃO: Janela compacta no TOPO (não fullscreen) ───────────
    val lp = WindowManager.LayoutParams(
